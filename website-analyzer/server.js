  ...
  try {
    ...
    for (const testUrl of urlVariants) {
      ...
      try {
        ...
        if (!response.ok()) {
          console.warn(
            `❌ CDX API returned ${response.status()} for ${testUrl}`
          );
          continue;
        }
        ...
        console.log(
          `📄 CDX Response length: ${textContent.length} characters for ${testUrl}`
        );

        // CDX形式のデータをパース
        const lines = textContent.trim().split('\n');
        console.log(
          `📊 Found ${lines.length} lines in CDX response for ${testUrl}`
        );
        ...
        if (lines.length > 1) {
          ...
          for (let i = 1; i < lines.length; i++) {
            try {
              ...
              console.warn(
                `❌ Failed to parse line ${i}: ${lines[i]} - Error: ${parseError.message}`
              );
              continue;
            }
          }
          ...
          if (snapshots.length > 0) {
            ...
            console.log(
              `✅ Found ${snapshots.length} snapshots for ${testUrl}`
            );
    ...
    const uniqueSnapshots = allSnapshots.filter((snapshot, index, arr) => {
      ...
      return (
        arr.findIndex((s) => s.timestamp.substring(0, 6) === yearMonth) ===
        index
      );
    });
    ...
    console.log(
      `🎉 Successfully processed ${finalSnapshots.length} unique snapshots from ${successfulUrl}`
    );
    console.log(
      `📅 Date range: ${formatTimestamp(finalSnapshots[finalSnapshots.length - 1]?.timestamp)} → ${formatTimestamp(finalSnapshots[0]?.timestamp)}`
    );
...
function determineChangeType(timestamp) {
  ...
  const monthsAgo = Math.floor(
    (currentTime.getTime() - snapshotTime.getTime()) /
      (1000 * 60 * 60 * 24 * 30)
  );
...
app.listen(PORT, () => {
  console.log(
    `🚀 Wayback Machine API Server running at http://localhost:${PORT}`
  );
  console.log(
    `📡 CDX API endpoint: http://localhost:${PORT}/api/wayback-snapshots/{url}`
  );
});
